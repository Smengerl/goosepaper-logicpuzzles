...
table.kakuro-grid .kakuro-h { position: absolute; bottom: 0; right: 2px; font-size: 0.5em; color: #000; }
table.kakuro-grid .kakuro-v { position: absolute; top: 0; left: 2px; font-size: 0.5em; color: #000; }
...
class PuzzleStoryProvider(StoryProvider):
    ...
    Constructor parameters:
      ...
      - `explanation` (optional, default "none"): "none"/"inline"/"footer"/"appendix" - see
        get_stories()'s docstring for what each mode renders.
        ...
        *does* show, per puzzle instance, is:
        ...
        for i in range(self.count):
            ...
            if self.explanation == "inline":
                puzzle_extra = f'<p class="puzzle-explanation-inline">{explanation_text}</p>'
                ...
                puzzle_extra = f'<sup class="puzzle-footnote-xref">{footnote_number}</sup>'
            ...
            puzzles.append(
                Story(
                    ...
                    body_html=(
                        _PUZZLE_CSS
                        + f'<div class="puzzle-body">{visible_label}{givens_html}{puzzle_extra}</div>'
                    ),
                    short_form=True,
                )
            )
