...
`style` selects a visual theme. `layout` controls the overall column density: with `"auto"`, Goosepaper defaults to a single reading column on narrow device profiles like `remarkable1`, `remarkable2`, and `paper_pro_move`, and to denser multi-column pages on larger profiles like `paper_pro`, `letter`, and `a4`. If you want to force it, set `"layout": "1col"`, `"2col"`, or `"3col"`. If you want a linked contents block near the top of the issue, set `"table_of_contents": true` in the `paper` object. If you want to override the body typeface without taking over the whole design, set `"body_font": "Literata"`. If you want to target a specific device or paper shape, set `"page_profile"` to one of `remarkable1`, `remarkable2`, `paper_pro`, `paper_pro_move`, `letter`, or `a4`. (`"rm1"` also works as a short alias.) RSS sources can also set `"byline": "all"`, `"none"`, or `"first"`, plus `"body_source": "auto" | "content" | "summary" | "article"`. RSS sources can also clean up fetched article HTML with `"content_skip_filters"` - a list of `{"type": "css", "selector": "..."}` (removes every matching element, e.g. ad blocks or cookie banners) or `{"type": "regex", "pattern": "...", "flags": "i"}` (strips matching text from the raw HTML, `flags` optional, any of `i`/`s`/`m`/`x`) rules, applied in order (every `regex` filter first, then every `css` filter) - and skip whole entries by title with `"skip_title_patterns"`, a list of regexes matched case-insensitively against the entry title (e.g. `["^anzeige:", "^sponsored"]` to drop sponsored posts before they're even fetched). Both have an "accept" counterpart for the inverse case - narrowing down instead of cleaning up: `"content_accept_filters"`, a list of `{"selector": "..."}` CSS-selector rules tried in order, keeping only the first matching element's contents instead of the whole parsed tree (useful when `readability`'s own extraction misses and you know exactly which container holds the real article; falls through unchanged if nothing matches, so a miss never zeroes out a story) - and `"accept_title_patterns"`, a list of regexes where only entries matching at least one are kept (e.g. `["amazon", "amzn"]` on an otherwise general business feed, to build a single-company news ticker). Bluesky sources can set `"include_replies": true | false`. Weather sources can set `"mode": "summary" | "hourly" | "daily" | "hourly_daily"` plus `hours`, `step_hours`, `days`, and `clock_format` for richer forecasts.
